...
(* [actor_to_string] returns corresponoing lowercase string version of type 
 * actor *)
...
(* [bop_to_string] returns string value of the build_card option, with "none"
 * for None and the name of the build_card for Some build_card *)
 ...
 * input. the int refering to a specific ai player starts from 1 because 0
 * is allocated to the human player. the ai personality is initialized
 * by parsing the ai_presets.json file *)
let ai_init num =
  let p_json_to_record p_json=
    ...
    {assassin = assassin;thief = thief; magician= magician;
     king = king;bishop=bishop;merchant=merchant;architect=architect;
     warlord=warlord}
  in
  let ai_init_helper pl n =
    {ai_name = n+1; personality = List.nth pl n; actors_before = [];
     actors_after = []}
  in
...
let ai_power (s:state) (ba: bot array) (n:int) =
 ...
 in
  ...
  |Assassin -> actor_to_string (assassin_target b s n)
  |Thief -> actor_to_string (thief_target s n b)
  |Magician -> magician_power n s
  |Architect -> bop_to_string (architect_power n s)
  |Warlord -> let target_pop = warlord_target n s in
     (let target_actor =
       (match target_pop with
        |None->Nil |Some x->x.role)
     in
     (actor_to_string target_actor)^" "
     ^(bop_to_string (building_to_destroy target_pop n s)))
  |Nil |King |Bishop |Merchant -> 
     failwith "ai_power should not be called for this actor"
...
let ai_update (s: state) (ba: bot array) =
    for i = 0 to ((Array.length ba) - 1) do
      ...
      Array.set ba i 
        {ai_name=b.ai_name; 
         personality= change_personality b.ai_name s b.personality;
