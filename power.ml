...
let rec power_thief (s: state) (p : player) (ba : bot array) : state =
  let cmd =
  ...
  in
    let robbed' = type_actor cmd in
     ...
     else if robbed' = s.killed
     then
       let () = robbed_is_dead () in power_thief s p ba
     else
      let () =
      ...
      (match robbed' with
      | Thief ->
        robbed_yourself (); {s with robbed = robbed'}
      | Magician | King | Bishop | Merchant | Architect | Warlord ->
        {s with robbed = robbed'}
      | Assassin ->
        thief_cant_choose_assassin (); power_thief s p ba
      | _ ->
        error_actor p.role; power_thief s p ba)

let rec power_assassin (s: state) (p : player) (ba : bot array) : state =
  let cmd =
    (if (p.is_ai) then help_job p.role true (ai_power s ba p.name) s
    else help_job p.role false " " s)
  in
  let killed' = type_actor cmd in
   if killed' = Nil then power_assassin s p ba
    ...
    (let () =
      (if (not(List.exists (fun x -> x.role= killed') s.players))then
       actor_not_in_round ()
       else ())
      in
      (match killed' with
      | Thief | King | Bishop | Merchant | Architect | Warlord | Magician ->
      {s with killed = killed'}
      |  _  ->
       error_actor p.role; power_assassin s p ba))

let rec power_magician (s:state) (p : player) (ba : bot array) : state =
  let command =
    if (p.is_ai) then help_job p.role true (ai_power s ba p.name) s
    else help_job p.role false " " s
  in
...
let power_king (s: state) (p:player):state =
...
let power_merchant (s: state) (p:player) =
  help_job_kbma p.role;
  let updated_merchant = {p with gold = p.gold + gold_inc p } in
...
(*[warlord_correct_actor] returns if the string is a valid actor *)
let rec warlord_correct_actor (s:state) (p:player) (cmd : string) =
    let victim= (try (type_actor cmd)
                with _ -> let () = warlord_incorrect_actor () in
                warlord_correct_actor s p (command (s))) in
    if (victim = Bishop) then
      let () = warlord_cant_target_bishop () in
...
(*[find_actor] returns the player of the  matching actor*)
...
(*[find_building] returns the build_card based on the string input  *)
let find_building (n:string) (p:player) (is_built:bool): build_card=
  let bl = (if (is_built) then p.buildings else p.hand) in
  (try (List.find(fun (x:build_card) ->
    (String.lowercase_ascii x.name) = n) bl)
  with Not_found ->
     failwith "Could not find building given the build_card name.")

let power_warlord (s : state) (p : player) (ba : bot array) : state=
  ...
  if cmd = "none" then s
    ...
    (let victim= warlord_correct_actor s p cmd in
    let rec helper (s: state) (victim:actor) : state  =
      ...
      let victim_rec = find_actor victim s in
      if (is_in_build (victim_rec.buildings) (building_name)) then
        (
          let building = find_building (building_name) victim_rec true in
          if (p.gold >=building.cost-1) then
        ( let victim_buildings =
          List.filter(fun (x:build_card) -> x<>building) victim_rec.buildings in
          let victim_points = victim_rec.points - (building.score) in
          let updated_victim = {victim_rec with buildings = victim_buildings;
...
let is_in_hand (p:player) (b_name:string) : bool =
  let rec is_in_hand_helper b (hl: build_card list)=
    (match hl with
    |[]  -> false
    |h::t-> if b = (String.lowercase_ascii h.name) then true
            else is_in_hand_helper b t)
  in
  is_in_hand_helper b_name (p.hand)

(*[get_hand] retruns a build_card that matches with the name and the string
 * string input. Returns build_card and inputs a build_card list and strings *)
let rec get_hand (lst: build_card list ) (cmd : string) :build_card  =
  match lst with
   | [] -> failwith "not in hand"
   | h :: t -> if (String.lowercase_ascii h.name) = cmd then h
               else get_hand t cmd

(*[match_hand] returns a bool that checks whether the  string input; thus the
 * build_card name is within the build_card list  *)
let rec match_hand (lst:build_card list ) (cmd:string) : bool =
  match lst with
   |  [] -> false
   | h :: t -> if (cmd = (String.lowercase_ascii h.name)) then true
               else match_hand t cmd
...
let rec build (p:player) (s:state) : state =
  ...
  if building_name = "none" then s
    ...
    if (p.gold >=building.cost)
      ...
      (
        let updated_hand =
        List.filter (fun (x:build_card) -> x<>building) p.hand in
        let updated_buildings =
        building::p.buildings in
        let updated_points =
        p.points + building.score in
        let updated_gold =
        p.gold - building.cost in
        let player =
        {p with hand = updated_hand; buildings = updated_buildings;
         points = updated_points; gold = updated_gold} in
        let plist  = replace s player in
        {s with players = plist}
      )
       else let () = cant_build_gold () in  build p s )

let rec power_architect (s:state) (p:player) (ba : bot array) : state =
  help_job_kbma p.role;
...
let power_player (p : player) (s: state) (ba: bot array) : state =
  let a  = p.role in
  match a with
